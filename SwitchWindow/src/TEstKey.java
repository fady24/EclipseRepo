...
public class DynamicSwitchWindowKeywordHandler implements KeywordHandler {
	...
	public KeywordResult process(WebDriver driver, HtmlObject htmlObject, ComponentModel componentModel, Map testData) {
		...
		try{
				...
				win = testData.get(componentModel.getParam1()).toString();
				
		winno = Integer.parseInt(win);
		...
		if (testData.get(componentModel.getParam3()) == null || testData.get(componentModel.getParam3()).equals(""))
				winTitle = "notitle";
		else
				winTitle =  testData.get(componentModel.getParam3()).toString();
	...
	{	t = (System.nanoTime()-start)/1000000000.0;
		...
		{	//System.out.println("All handles: "+ whandles);
			...
			driver.switchTo().window(warray[winno-1]);
				...
				if (winTitle==driver.getTitle().toString() || winTitle.equals(driver.getTitle().toString()) || winTitle == "notitle")
				{	flag = 1;	
